...
const Step1 = ({ onNext }) => {
  ...
  const [adoptionData, setAdoptionData] = useState({}); // Verileri tutacak değişken
  ...
  const handleNext = () => {
    // İlk adımda veri tutma
    if (activeStep === 1) {
      setAdoptionData(prevData => ({ ...prevData, animalType }));
    }

    // Diğer adımlar için veri ekleme
    if (activeStep < steps.length) {
      onNext(adoptionData); // Veriyi bir üst adımla ilet
      setActiveStep(activeStep + 1);
    } else {
      console.log("Ad creation process", adoptionData); // Son adımda tüm veriler
